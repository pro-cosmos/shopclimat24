...
class ModelExtensionModuleQuickpay extends Model {
	...
	public function getQuickpay() {
		
		$res0 = $this->db->query("SHOW TABLES LIKE '".DB_PREFIX."quickpay'");
		if($res0->num_rows != 0){
		$query = $this->db->query("SELECT * FROM ". DB_PREFIX ."quickpay"); 

		return $query->rows;
		}
	}
	
	public function deletequickpay($id) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "quickpay` WHERE `id` = '" . (int)$id . "'");
		
	}
	
	public function updatestatusquickpay($id, $status) {
		$this->db->query("UPDATE " . DB_PREFIX . "quickpay SET status = '" . (int)$status . "' WHERE id = '" . (int)$id . "'");
	}
